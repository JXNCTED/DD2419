...
class CartesianController(Node):
    def __init__(self):
        ...
        self.twist_sub_ = self.create_subscription(Twist, '/motor_controller/twist', self.twist_callback, 10)
        self.encoder_sub_ = self.create_subscription(Encoders, '/motor/encoders', self.encoder_callback, 10)
        self.duty_cycle_pub_ = self.create_publisher(DutyCycles, '/motor/duty_cycles', 10)
    ...
    def encoder_callback(self, msg):
        ...
        vw1 = (msg.delta_encoder_left * 2 * pi) / TICK_PER_REV * WHEEL_RADIUS / DT
        vw2 = (msg.delta_encoder_right * 2 * pi) / TICK_PER_REV * WHEEL_RADIUS / DT
        ...
        self.int_error_wheel1 = min(max(self.int_error_wheel1, -0.3), 0.3)
        self.int_error_wheel2 = min(max(self.int_error_wheel2, -0.3), 0.3)

        pwm_1 = min(max(KP * error_wheel1 + KI * self.int_error_wheel1, -1.0), 1.0)
        pwm_2 = min(max(KP * error_wheel2 + KI * self.int_error_wheel2, -1.0), 1.0)
