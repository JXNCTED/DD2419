...
class Workspace(Node):
    ...
    def __init__(self):
        ...
        self.workspace_publisher_marker = self.create_publisher(
            Marker, 'workspace/boundary', 10)
        self.timer_workspace = self.create_timer(
            1, self.publish_workspace_boundary)
        ...
        self.check_publisher = self.create_publisher(
            Bool, 'workspace/boundary_check/response', 10)

        # Service where request of specific point is answered with True or False
        # SERVICE IS THE WAY TO GO!!
        self.srv = self.create_service(
            IsInWorkspace, 'is_in_workspace', self.is_point_inside_workspace_service)
    ...
    def publish_workspace_boundary(self):
        ...
        marker = Marker()
        # Set the frame in which the polygon will be displayed
        ...
        marker.points = [Point(x=point.x, y=point.y, z=point.z)
                         for point in self.WORKSPACE.polygon.points]
    ...
    def is_point_inside_workspace(self, point):
        ...
        ray_start = point
        # Extend ray to a very large x value
        ray_end = Point32(x=1e6, y=point.y, z=0.0)
    ...
    def is_point_inside_workspace_service(self, request, response):
        ...
        response.is_in_workspace = is_inside
        return response
